import React from "react";

export const Partners = () => {
    return (
        <div className="flex justify-between items-center w-full">
            <img className="h-[40px]" src="/images/main/partners/bando.png"/>
            <img className="h-[40px]" src="/images/main/partners/mansion.png"/>
            <img className="h-[40px]" src="/images/main/partners/kritika.png"/>
            <img className="h-[40px]" src="/images/main/partners/ayne.png"/>
            <img className="h-[40px]" src="/images/main/partners/junky.png"/>
            <img className="h-[40px]" src="/images/main/partners/stain.png"/>
        </div>
    )
}